...
const HeroSection = () => {
  ...
  const [filteredMaterials, setFilteredMaterials] = useState([
    { name: 'Building Materials', image: '/images/image (14).png' },
    { name: 'Finishing Materials', image: '/images/image (15).png' },
    { name: 'Hardware and Tools', image: '/images/hardware_tools-removebg-preview.png' }
  ]);
    ...
    <div className="flex flex-col bg-white py-4 px-2 sm:px-4 md:px-6 lg:px-8 xl:px-12 w-full">
      ...
      <div className="flex items-center justify-between w-full mb-4">
        <div className="flex items-center space-x-2 mx-auto md:mx-0">
          <Image src="/images/image (22).png" alt="Logo" width={120} height={40} className="w-20 sm:w-28 h-auto" />
        </div>

        <div className="relative flex items-center space-x-3 sm:space-x-4 md:space-x-6 mx-auto md:mx-0">
          <FiBell
            className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-gray-600 cursor-pointer"
    ...
    {loading ? (
      ...
      limitedHotDeals.map((deal) => (
        <div key={deal.id} className="flex flex-col justify-center items-center bg-white border border-gray-200 rounded-lg shadow-lg p-4">
          <Image
            src={deal.image || placeholderImage}
            alt={deal.name} 
            width={150} 
            height={150}
